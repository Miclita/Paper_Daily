...
- **类别**: 医学影像 AI
- **相关性**: 8/10 — 深度学习用于医学影像（X光）的骨密度预测，直接服务于临床评估，AI是核心贡献。
- **原文链接**: https://pubmed.ncbi.nlm.nih.gov/41944722/

**中文摘要**: 背景：测量骨密度（BMD）对于评估儿童骨骼健康至关重要。双能X线吸收测定法（DXA）是参考标准，但其可及性有限。目的：开发并评估一种人工智能模型，用于从儿童胸部X光片预测BMD。材料与方法：这项回顾性研究纳入了在两家三级医院接受DXA和胸部X光检查且时间间隔不超过3个月的年龄小于18岁的患者（内部测试）。
...
**中文摘要**: 背景 基于模板的PET指标可以量化阿尔茨海默病（AD）的β淀粉样蛋白（Aβ）和tau蛋白负荷，但将全脑数据压缩为单一标量值，忽略了疾病的异质性，有时会导致影像与临床表现不一致。人工智能（AI）方法可以捕捉更丰富的模式，但通常缺乏生物学可解释性。目的 开发并验证一种可解释的深度学习框架，通过病理生理学机制将AD特异性异常与生理性摄取区分开来。
