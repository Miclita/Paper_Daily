...
| 13 | 医疗应用 | 利用人工智能推动基于CRISPR的基因组编辑技术发展 | Nature reviews. Genetics | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41254174/) |
...
材料与方法：本研究为一项回顾性、多中心研究。研究者基于1958年6月至2019年2月间采集的33 901例三维胸部CT扫描数据，采用自监督学习方法构建了“统一胸部CT肺癌影像基础模型”（Unified CT-Based Lung Cancer Imaging Foundation, UCLIF）。该模型首先通过对比式掩码图像建模（contrastive masked image modeling）任务进行预训练，随后针对肺部相关任务进行微调。
...
材料与方法：本研究为一项回顾性多中心研究（2021年1月至2023年11月），纳入经影像学检查提示为“性质未定”的卵巢肿瘤患者。数据集分为训练集、内部验证集（n = 360）和外部测试集（n = 27）。最终组织病理学诊断结果作为参考标准。
...
**中文摘要**: 目的：评估基于成人患者训练的肝母细胞瘤分割模型在儿童患者中的泛化能力，并分别针对儿童增强CT和T2加权MRI影像，构建两种深度学习（DL）模型——M<sub>PCT</sub>（Pediatric Contrast-enhanced CT）和M<sub>pMRI</sub>（Pediatric MRI）。  

材料与方法：本研究分析了多中心儿童肿瘤学组（Children’s Oncology Group, COG）AHEP0731临床试验（NCT00980460；2008年5月—2018年7月）的影像数据。所采用的深度学习模型均基于三维U-Net网络架构进行训练……
...
材料与方法：本研究为一项回顾性、双中心研究，纳入2019年1月至2021年12月期间接受增强腹部CT检查的CRC患者。DLA的诊断准确性分别在病灶水平（per-nodule level）和患者水平（per-patient level）上进行评估，并与放射科医师的诊断结果进行对比。
...
**中文摘要**: 基础模型（尤其是大语言模型）的引入，为医疗健康领域带来了变革性前景。然而，该领域正迅速向自主智能体系统（autonomous agent systems）演进。此类系统被定义为能够感知环境并作出响应以实现特定目标的人工智能（AI）实体，标志着临床辅助范式从被动式信息检索向主动式、目标导向型临床支持的根本性转变。智能体驱动的AI系统通过其核心能力——包括规划（planning）、工具调用（tool use）、记忆（memory）及反思（reflection）——突破了静态知识的固有局限。
...
**中文摘要**: 目的：构建一种自监督的文本—视觉联合学习框架，利用免费获取的神经放射学报告文本，实现脑部MRI扫描图像异常的自动检测，从而避免对专家标注训练数据集的依赖。  
...
材料与方法：回顾性收集2017年3月至2025年4月期间获取的CT扫描数据（共计189 710次扫描，涉及106 563名患者），来源包括本机构影像归档与通信系统（PACS）及两个外部数据集（分别包含19 393名和1158名患者）。
