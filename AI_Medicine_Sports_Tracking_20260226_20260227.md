...
| # | 类别 | 中文标题 | 期刊 | 链接 |
| :--- | :--- | :--- | :--- | :--- |
| 1 | 医疗应用 | 将常规CT检查转化为定量放射学的参考值 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41670446/) |
| 2 | 视觉/核心AI (医疗/运动相关) | 标注层级对多序列MRI模型术前预测肝细胞癌微血管侵犯的影响 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41718534/) |
| 3 | 医疗应用 | 教MRI“阅读”报告：面向大规模脑部MRI的文本—图像对齐 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41636622/) |
| 4 | 视觉/核心AI (医疗/运动相关) | 一种基于三维胸部CT扫描的自监督基础模型用于肺癌诊断与预后预测 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41718531/) |
| 5 | 医疗应用 | 基于深度学习的人工智能模型在多中心CT数据集上对卵巢肿瘤分类性能的评估 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41686074/) |
| 6 | 医疗应用 | 面向儿童的个性化深度学习模型用于肝母细胞瘤在CT和MRI影像中的分割 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41718532/) |
| 7 | 医疗应用 | RSNA乳腺筛查X线摄影癌症检测挑战赛的开源数据集 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41563075/) |
| 8 | 医疗应用 | 一种用于结直肠癌患者增强腹部CT中肝转移灶检测的深度学习算法：与放射科医师的对比... | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41649392/) |
| 9 | 医疗应用 | 利用人工智能推动基于CRISPR的基因组编辑技术发展 | Nature reviews. Genetics | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41254174/) |
...
| 12 | 医疗应用 | 基于自监督文本-视觉对齐的自动化脑部MRI异常检测：一项多中心研究（ALIGN研... | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41295086/) |
| 13 | 医疗应用 | 基于CT图像分割的全生命周期人体解剖图谱：大规模横断面与纵向分析 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41441443/) |
| 14 | 医疗应用 | 人工智能增强型超声心动图在心血管疾病管理中的应用 | Nature reviews. Cardiology | [PubMed](https://pubmed.ncbi.nlm.nih.gov/40764834/) |
| 15 | 医疗应用 | 多样化的数据库与机器学习模型以缩小RNA结构预测中的泛化差距 | Science advances | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41739924/) |

---

## 3. 文献详情

### 1. 将常规CT检查转化为定量放射学的参考值
...
### 2. 标注层级对多序列MRI模型术前预测肝细胞癌微血管侵犯的影响
...
**中文摘要**: 目的：评估融合多模态数据的深度学习模型在肝细胞癌微血管侵犯（microvascular invasion, MVI）预测中的性能，并探究不同人工标注方法对模型性能的影响。  
...
### 3. 教MRI“阅读”报告：面向大规模脑部MRI的文本—图像对齐
...
### 4. 一种基于三维胸部CT扫描的自监督基础模型用于肺癌诊断与预后预测
...
材料与方法：本研究为一项回顾性、多中心研究。研究者基于1958年6月至2019年2月间采集的33 901例三维胸部CT扫描数据，采用自监督学习范式构建了“统一胸部CT肺癌影像基础模型”（Unified CT-Based Lung Cancer Imaging Foundation, UCLIF）。该模型首先通过对比式掩码图像建模（contrastive masked image modeling）任务进行预训练，随后针对肺部相关下游任务进行微调。
...
### 5. 基于深度学习的人工智能模型在多中心CT数据集上对卵巢肿瘤分类性能的评估
...
**中文摘要**: 目的：构建一种基于深度学习的计算机辅助诊断（CADx）模型，用于术前在CT扫描图像上对卵巢肿瘤（OTs）进行分类，并将其性能与当前超声（US）模型及放射科医师评估结果进行比较。  
材料与方法：本研究为一项回顾性多中心研究（2021年1月至2023年11月），纳入经影像学评估为“性质不明”的卵巢肿瘤患者。数据集分为训练集、内部验证集（n = 360）和外部测试集（n = 27）。最终组织病理学诊断结果作为参考标准。
...
### 6. 面向儿童的个性化深度学习模型用于肝母细胞瘤在CT和MRI影像中的分割
...
### 8. 一种用于结直肠癌患者增强腹部CT中肝转移灶检测的深度学习算法：与放射科医师的对比研究
...
材料与方法：构建了一个模拟框架，用于评估LLM引入的标签错误对模型性能估计值的影响。基于低患病率（10% 和 30%）及高患病率（70% 和 90%）两类设定，生成了包含10,000例样本的合成数据集。LLM的敏感度（sensitivity）与特异度（specificity）被独立设置为不同水平，并据此模拟其产生的标签噪声。
...
### 12. 基于自监督文本-视觉对齐的自动化脑部MRI异常检测：一项多中心研究（ALIGN研究）
...
### 13. 基于CT图像分割的全生命周期人体解剖图谱：大规模横断面与纵向分析
...
**中文摘要**: 目的：构建基于CT影像的全身体解剖容积在成年期的分布模型，并建立全面的横断面与纵向参考图表，以弥补当前缺乏非脑部CT影像为基础的全身体积标准的空白。  
...
**中文摘要**: 人工智能（AI）正在深刻变革超声心动图学，推动该领域迈入诊断精准度更高、检查效率更优、患者照护质量更佳的新时代。本文综述系统深入地探讨了人工智能在超声心动图中的应用，重点阐述了该领域的最新研究进展、临床实践现状及未来发展趋势。我们详细梳理了人工智能在超声心动图全流程中的整合路径，涵盖图像采集、定量分析到结果判读等各个环节。同时，本文亦概述了人工智能在自动化常规……
...
### 15. 多样化的数据库与机器学习模型以缩小RNA结构预测中的泛化差距
...
**中文摘要**: 解析蛋白质与核酸等大分子的三维结构，对于阐明其功能及生物学作用至关重要。X射线晶体学、核磁共振波谱学和冷冻电子显微镜等先进技术的发展，已推动超过18万种蛋白质结构的解析工作，并全部收录于蛋白质数据银行（Protein Data Bank, PDB）中。这一全面、权威的结构数据库，为开发基于深度学习的蛋白质结构预测算法提供了关键支撑，使研究人员得以直接从氨基酸序列出发，高精度地预测蛋白质的三维构象。相比之下……
