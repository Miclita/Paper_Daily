...
| 7 | 医疗应用 | 面向儿童的个性化深度学习模型用于肝母细胞瘤在CT和MRI影像中的分割 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41718532/) |
...
材料与方法：本研究为一项回顾性多中心研究（2021年1月至2023年11月），纳入经影像学评估为“性质未定”的卵巢肿瘤患者。数据集分为训练集、内部验证集（n = 360）和外部测试集（n = 27）。最终组织病理学诊断结果作为参考标准。
...
### 7. 面向儿童的个性化深度学习模型用于肝母细胞瘤在CT和MRI影像中的分割
...
材料与方法：构建了一个模拟框架，用于评估LLM引入的标签错误对模型性能估计结果的影响。基于低患病率（10%和30%）及高患病率（70%和90%）两种条件，生成了包含10 000例样本的合成数据集。LLM的敏感度（sensitivity）与特异度（specificity）被独立设置为不同水平，并予以系统性变异。
...
**中文摘要**: 基于CRISPR的基因组编辑技术——包括核酸酶介导的编辑、碱基编辑和先导编辑——通过实现基因组的精准、可编程修饰，彻底变革了生物学研究与现代医学，并为多种遗传性疾病提供了全新的治疗策略。人工智能（AI），特别是机器学习与深度学习模型，正进一步推动该领域的发展：一方面加速针对多样化靶点的基因编辑工具的优化；另一方面指导……
...
**中文摘要**: 人工智能（AI）正在深刻变革超声心动图学，推动该领域迈入诊断精准度更高、检查效率更优、患者照护质量更佳的新时代。本文综述系统深入地探讨了人工智能在超声心动图中的应用，重点阐述了该领域的最新研究进展、临床实践现状及未来发展趋势。我们详细梳理了人工智能在超声心动图全流程中的整合路径，涵盖图像采集、定量分析至结果判读等各个环节。同时，本文概述了人工智能在自动化常规……
