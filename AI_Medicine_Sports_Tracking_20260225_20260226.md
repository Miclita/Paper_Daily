...
| 3 | 视觉/核心AI (医疗/运动相关) | 一种基于三维胸部CT扫描的自监督基础模型用于肺癌诊断与预后预测 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41718531/) |
| 4 | 医疗应用 | 教MRI“阅读”报告：面向大规模脑部MRI的文本—图像对齐 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41636622/) |
| 5 | 医疗应用 | 面向儿童的个性化深度学习模型用于肝母细胞瘤在CT及MRI影像中的分割 | Radiology. Imaging cancer | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41718532/) |
...
| 10 | 医疗应用 | 大型语言模型生成标注中的标签噪声对诊断模型性能评估的影响 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41441444/) |
| 11 | 视觉/核心AI (医疗/运动相关) | 放射学中的自主型人工智能：从大语言模型到未来临床整合的演进 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41532836/) |
| 12 | 医疗应用 | 基于CT图像分割的全生命周期人体解剖图谱：大规模横断面与纵向分析 | Radiology. Artificial intelligence | [PubMed](https://pubmed.ncbi.nlm.nih.gov/41441443/) |
...
### 3. 一种基于三维胸部CT扫描的自监督基础模型用于肺癌诊断与预后预测
...
材料与方法：本研究为一项回顾性多中心研究。研究者基于1958年6月至2019年2月间采集的33 901例三维胸部CT扫描数据，采用自监督学习范式构建了“统一胸部CT肺癌影像基础模型”（Unified CT-Based Lung Cancer Imaging Foundation, UCLIF）。该模型首先通过对比式掩码图像建模（contrastive masked image modeling）任务进行预训练，随后针对肺部相关下游任务进行微调。
...
### 5. 面向儿童的个性化深度学习模型用于肝母细胞瘤在CT及MRI影像中的分割
...
**中文摘要**: 目的：评估基于成人患者训练的肝母细胞瘤分割模型在儿童患者中的泛化能力，并分别针对儿童增强CT和T2加权MRI影像，构建两种深度学习（DL）模型——M<sub>PCT</sub>与M<sub>pMRI</sub>。  
材料与方法：本研究分析了多中心儿童肿瘤学组AHEP0731临床试验（NCT00980460；2008年5月—2018年7月）的影像学数据。所采用的深度学习模型基于三维U-Net架构进行训练……
...
材料与方法：本研究为一项回顾性多中心研究（2021年1月至2023年11月），纳入经影像学检查提示为“不确定性质”的卵巢肿瘤患者。数据集分为训练集、内部验证集（n = 360）和外部测试集（n = 27）。最终组织病理学诊断结果作为参考标准。
...
**中文摘要**: 目的：评估一种深度学习算法（DLA）在结直肠癌（CRC）患者中检测肝转移灶（LM）的性能，涵盖多样化的临床场景，并将其诊断准确性与放射科医师进行比较。  
...
**中文摘要**: 基于CRISPR的基因组编辑技术——包括核酸酶介导的编辑、碱基编辑和先导编辑——通过实现基因组的精准、可编程修饰，彻底变革了生物学研究与现代医学，并为多种遗传性疾病提供了全新的治疗策略。人工智能（AI），特别是机器学习与深度学习模型，正进一步推动该领域的发展：一方面加速针对多样化靶点的基因编辑器的优化；另一方面……
...
### 10. 大型语言模型生成标注中的标签噪声对诊断模型性能评估的影响
...
材料与方法：构建了一个模拟框架，用于评估LLM引入的标签错误对模型性能估计结果的影响。基于低患病率（10%和30%）及高患病率（70%和90%）两种条件，生成了包含10 000例样本的合成数据集。LLM的敏感度（sensitivity）与特异度（specificity）被独立设置为不同水平，并据此模拟其产生的标签噪声。
...
### 11. 放射学中的自主型人工智能：从大语言模型到未来临床整合的演进
...
**中文摘要**: 基础模型（尤其是大语言模型）的引入，为医疗健康领域带来了变革性潜力。然而，该领域正迅速向自主智能体系统（autonomous agent systems）演进；此类系统被定义为能够感知环境并作出响应以实现特定目标的人工智能（AI）实体，标志着临床辅助范式从被动式信息检索向主动式、目标导向型临床支持的根本性转变。智能体式AI系统通过其核心能力——包括规划（planning）、工具调用（tool use）、记忆（memory）及反思（reflection）——突破了静态知识的固有局限。
...
### 12. 基于CT图像分割的全生命周期人体解剖图谱：大规模横断面与纵向分析
...
**中文摘要**: 目的：构建基于CT影像的全身体解剖容积在成年期的分布模型，并建立全面的横断面与纵向参考图表，以弥补当前缺乏基于非脑部CT影像的全身体积标准的空白。  
...
**中文摘要**: 目的：构建一种自监督的文本—视觉联合学习框架，利用免费获取的神经放射学报告文本，实现脑部MRI扫描图像中异常征象的自动检测，从而避免对专家标注训练数据集的依赖。  
材料与方法：本研究为一项回顾性与前瞻性相结合的多中心研究，纳入2008年1月至2019年12月期间英国国家医疗服务体系（NHS）两家医院收治的成年患者所完成的81,936例脑部MRI检查及其对应的放射学报告，用于模型训练及内部验证。
